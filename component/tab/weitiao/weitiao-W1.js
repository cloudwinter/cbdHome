// component/weitiao/weitiao-W1.js
...
Component({
  ...
  data: {
    ...
    currentAnjian: {
      anjian: 'toubutz', // beibutz,tunbutz,toubutz,tuibutz
      name: '头部调整' // 背部调整，臀部调整，头部调整，腿部调整
    },
    currentXHAnjian: {
      anjian: 'toubuxh', // quanshengxh,tunbuxh,toubuxh,tuibuxh
      name: '头部循环' // 全身循环，臀部循环，头部循环，腿部循环
    },
  ...
  lifetimes: {
    created: function () {
      // 在组件实例刚刚被创建时执行
      console.info("weitiao-w1-->created");
      var that = this;
  ...
  methods: {
    ...
    initConnected(connected) {
      ...
      console.info('weitiao-W1->initConnected:', connected, this.observer);
      that.setData({
        connected: connected,
      })
      //WxNotificationCenter.removeNotification("INIT", that);
    ...
    touchStart(e) {
      ...
      if (type == 'beibutzTop') {
        ...
        this.donghua(true, this);
        this.tapTunbutz(true, true);
      } else if (type == 'tunbutzBottom') {
        this.setData({
          currentAnjian: {
            anjian: 'tunbutz',
            name: '臀部调整'
    ...
    touchEnd(e) {
      ...
      if (type == 'beibutzTop') {
        ...
        this.tapTunbutz(true, false);
      } else if (type == 'tunbutzBottom') {
        this.setData({
          tunbutzBottom: false
        });
        this.tapTunbutz(false, false);
      } else if (type == 'toubutzTop') {
        this.setData({
          toubutzTop: false
        });
